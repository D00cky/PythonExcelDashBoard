...
from app.routes.main import _cached_polo_context, _defer_plotly_script
...
def test_cache_reuses_context_for_same_path_and_filter(tmp_path):
    ...
    _cached_polo_context.cache_clear()

    _cached_polo_context(str(path), path.stat().st_mtime_ns, "", "", False)
    _cached_polo_context(str(path), path.stat().st_mtime_ns, "", "", False)

    info = _cached_polo_context.cache_info()
...
def test_cache_treats_different_filters_as_different_entries(tmp_path):
    ...
    _cached_polo_context(str(path), path.stat().st_mtime_ns, "", "", False)
    _cached_polo_context(str(path), path.stat().st_mtime_ns, "2026-03-10", "", False)
    _cached_polo_context(str(path), path.stat().st_mtime_ns, "", "2026-03-20", False)
    _cached_polo_context(str(path), path.stat().st_mtime_ns, "", "", True)
...
def test_cache_invalidates_when_file_mtime_changes(tmp_path):
    ...
    _cached_polo_context(str(path), first_mtime, "", "", False)
    ...
    _cached_polo_context(str(path), new_mtime, "", "", False)
